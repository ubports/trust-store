...
using PrimaryPidResolver = core::trust::Pid(const std::string&, core::trust::Pid);
class MockPrimaryPidResolver: public testing::MockFunction<PrimaryPidResolver> {
public:
    MockPrimaryPidResolver() {
        using namespace testing;

        ON_CALL(*this, Call(_, _)).WillByDefault(ReturnArg<1>());
    }
    ...
    std::function<PrimaryPidResolver> GetFunction()
    {
        return [this] (const std::string& app_id, core::trust::Pid requesting_pid)
        {
            return this->Call(app_id, requesting_pid);
        };
    }
};
...
std::shared_ptr<MockPrimaryPidResolver> a_mocked_primary_pid_resolver()
{
    return std::make_shared<MockPrimaryPidResolver>();
}
}
...
TEST(MirAgent, creates_prompt_session_and_execs_helper_with_preauthenticated_fd)
{
    ...
    auto primary_pid_resolver = a_mocked_primary_pid_resolver();
    ...
    core::trust::mir::Agent agent
    {
        core::trust::mir::Agent::Configuration
        {
            ...
            app_info_resolver,
            primary_pid_resolver->GetFunction(),
        }
    };
...
TEST(MirAgent, resolves_primary_pid_and_create_prompt_session_with_it)
{
    using namespace ::testing;

    const core::trust::Pid app_pid {21};
    const core::trust::Pid child_pid {42};
    const std::string app_icon{"/tmp"};
    const std::string app_name {"Does not exist"};
    const std::string app_id {"does.not.exist.application"};
    const core::trust::Feature feature{42};
    ...
    // Here's the twist: the requesting process is actually a child of the app.
    EXPECT_CALL(*primary_pid_resolver, Call(app_id, child_pid)).Times(1)
            .WillRepeatedly(Return(app_pid));
